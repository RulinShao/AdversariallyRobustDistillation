...
def get_args():
    ...
    parser.add_argument('--description', default='V6-2:Implemented fast kdiga with gradient alignment', type=str)
...
def main():
    ...
    for epoch in range(args.epochs):
        ...
        for i, (X, y) in enumerate(train_loader):
            ...
            loss = F.cross_entropy(output_s_adv, y)
            ...
            # Compute CE Loss on clean images
            ...
            grad_t_clean = grad_t_clean.detach()
            ...
            t_correct_ = t_correct[:, None, None, None].repeat([1]+list(grad_t_clean.shape[1:]))
            ...
            output_s_adv = model(X + delta1[:bs])
            ce_loss_adv = criterion(output_s_adv, y)
            with amp.scale_loss(loss, opt) as scaled_loss:
                grad2 = torch.autograd.grad(ce_loss_adv, delta1, create_graph=False)[0]
                grad2 /= scaled_loss / loss
            grad2 = grad2.detach()
